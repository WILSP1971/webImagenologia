...
"Invoke-RestMethod -Method Post http://localhost:8042/modalities/pacs/echo")
    ...
    h3("11.6 Visor Stone Web Viewer bajo el sub-path (validado)")
    p("Se eligió el <b>Stone Web Viewer</b> integrado (v2.5, cero build). Se expone como otra <b>app hija IIS</b> con la carpeta nombrada EXACTAMENTE <font face='Courier' size=9>stone-webviewer</font> (para que los recursos relativos de Stone —js/wasm/css— resuelvan bajo el mismo sub-path). Su <font face='Courier' size=9>web.config</font> reenvía <font face='Courier' size=9>^(.*)</font> → <font face='Courier' size=9>http://localhost:8042/stone-webviewer/{R:1}</font> con las mismas server variables X-Forwarded. Las llamadas DICOMweb de Stone caen en <font face='Courier' size=9>/PortalImagenologia/dicomweb/</font> (ya proxied).")
    callout("Validado END-TO-END (2026-08-07)", "Se subió un estudio de prueba por Orthanc Explorer (Upload) y se abrió por HTTPS en Stone: <b>la imagen se renderiza correctamente</b> (Navegador→IIS/ARR→Stone+DICOMweb→Orthanc). La app .NET abre estudios con <font face='Courier' size=9>.../stone-webviewer/index.html?study={StudyInstanceUID}</font> (motor='stone' en Abrir.cshtml).")
    danger("Gotcha del nombre 'dicom-web' (con guion)", "Stone asume que el DICOMweb es la carpeta HERMANA <font face='Courier' size=9>dicom-web</font> (con guion) al lado de <font face='Courier' size=9>stone-webviewer</font>. Por eso el <font face='Courier' size=9>DicomWeb.Root</font> debe ser <font face='Courier' size=9>/PortalImagenologia/dicom-web/</font> (con guion) y la app hija del proxy llamarse <font face='Courier' size=9>dicom-web</font>. Con <font face='Courier' size=9>dicomweb</font> (sin guion) Stone da 404 en <font face='Courier' size=9>/dicom-web/studies</font> y se queda cargando. Además: <font face='Courier' size=9>?study=</font> admite UN solo StudyInstanceUID (no pegar dos con espacio/%20).")
